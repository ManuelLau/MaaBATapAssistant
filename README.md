...
- 支持模拟器官服、模拟器B服、模拟器国际服(繁体中文)、模拟器日服、PC端国际服(繁体中文)、PC端日服
...
3. 设置模拟器/PC端路径或者每次在启动任务前手动打开模拟器/PC端（暂时只支持连接一个模拟器，默认连接最开始打开的模拟器），并确保在该模拟器/PC端上已登录好游戏账号

4. 将模拟器/PC端分辨率设置为1280\*720**或更高**的**16:9**的分辨率，例如1920\*1080、2560\*1440等
...
8. 如果需要使用“完成当前任务后退出模拟器/PC端”的功能，请正确填写模拟器/PC端路径，否则将无法正确关闭。**对于MuMu、雷电模拟器，会默认关闭索引为0的模拟器**
...
4. 对于PC端用户，使用过程中请尽量不要操作鼠标，以免产生冲突导致任务执行失败

5. 日服PC端用户需要使用管理员权限运行小助手，否则无法执行任务
